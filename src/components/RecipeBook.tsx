    ...
    <div className="relative">
            ...
            >
              ...
              <div className="bg-amber-100 p-3 border-b border-amber-200">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-bold text-amber-800">Recipe Collection</h3>
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                  ...
                  {RECIPES.map((recipe, index) => {
                    ...
                    return (
                      <motion.div
                        ...
                        whileHover={canCook && !isStoveOccupied ? { scale: 1.02 } : {}}
                        whileTap={canCook && !isStoveOccupied ? { scale: 0.98 } : {}}
                      >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="mt-2 flex flex-wrap gap-1">
                              {recipe.ingredients.slice(0, 4).map((ingredient) => {
                                const item = state.inventory.find(inv => inv.id === ingredient.ingredientId);
                                const hasEnough = item && item.quantity >= ingredient.quantity;
      ...
      >
        <div className="text-lg font-bold text-purple-600 flex items-center space-x-2">
          <span>📚</span>
          <span>Legendary Recipe Scrolls</span>
          <span>📚</span>
        </div>
        <motion.div 
          ...
          transition={{ duration: 2, repeat: Infinity }}
        >
          ✨ Click to discover magical recipes! ✨
        </motion.div>
      </motion.div>
    </div>
  );
